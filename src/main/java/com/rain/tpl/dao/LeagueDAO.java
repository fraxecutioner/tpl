...
public class LeagueDAO {
	...
	public boolean createLeague(League tempLeague, Team tempTeam, Player tempPlayer) {
		...
		SessionFactory factory = new Configuration().addAnnotatedClass(Player.class).addAnnotatedClass(Team.class)
				.addAnnotatedClass(League.class).buildSessionFactory(new StandardServiceRegistryBuilder()
	...
	public List<PremierLeagueFixtures> getPlFixturesList(String gameWeek) {
		...
		try {
			...
			for (int i = 0; i < leagueObjList.size(); i++) {
				...
				PremierLeagueFixtures obj = new PremierLeagueFixtures((leagueObjList.get(i))[0].toString(),
						(leagueObjList.get(i))[1].toString(), null);
	...
	@Transactional
	public List<Team> joinedLeagueList(String ownerKey) {
		SessionFactory factory = new Configuration().addAnnotatedClass(Player.class).addAnnotatedClass(Team.class)
				.addAnnotatedClass(League.class).buildSessionFactory(new StandardServiceRegistryBuilder()
		...
		try {
			...
			List<Object[]> leagueObjList = new ArrayList<Object[]>();
			List<Team> leagueList = new ArrayList<Team>();
			ProcedureCall procedureCall = session.createStoredProcedureCall("usp_get_joinedleaguelist");
			procedureCall.registerParameter(1, Class.class, ParameterMode.REF_CURSOR);
			procedureCall.registerParameter(2, String.class, ParameterMode.IN);
			...
			for (Object[] row : leagueObjList) {
				Team t= new Team();
				t.setTeamName(row[0].toString());
				t.setTeamId(row[1].toString());
				leagueList.add(t);
			}
